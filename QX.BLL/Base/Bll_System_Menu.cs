...
namespace QX.BLL
{
   ...
   public partial class Bll_System_Menu
   {
        ...
        /// <returns>list</returns>
        public List<System_Menu> GetListByCode(string strCondition)
        {
        ...
        /// <param name='strCondition'>条件(AND Code='11')</param>
        /// <returns>model</returns>
        public System_Menu GetModel(int id)
        {
            System_Menu model = instance.GetByKey(id);          
            return model;
        }    
        ...
        /// <param name='model'>实体</param>
        /// <returns>bool</returns>
        public bool Update(System_Menu model)
        {            
            bool result = false;
            var e = new ModelExceptions();
        ...
        {
            bool result = false;
            List<System_Menu> list = instance.GetListByWhere(Condition);
            if (list.Count > 0)
            {
                System_Menu model = list[0];
                model.Stat = 1;
                int _rseult = instance.Update(model);
                if (_rseult > 0)
                {
