...
{
    [Serializable]
    public partial class Bll_Sys_Role
    {

        private ADOSys_Role instance = new ADOSys_Role();
        ...
        {
            List<Sys_Role> list = instance.GetAll();
            return list;
        }
        ...
        /// <returns>list</returns>
        public List<Sys_Role> GetListByCode(string strCondition)
        {
        ...
        {
            Sys_Role model = instance.GetByKey(id);
            return model;
        }
        ...
        /// <param name='model'>实体</param>
        /// <returns>bool</returns>
        public bool Update(Sys_Role model, bool IsValid)
        {
        ...
        {
            bool result = false;
            List<Sys_Role> list = instance.GetListByWhere(Condition);
            if (list.Count > 0)
            {
                Sys_Role model = list[0];
                model.Stat = 1;
                int _rseult = instance.Update(model);
                if (_rseult > 0)
                {
