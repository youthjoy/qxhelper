...
namespace QX.DAL
{
   ...
   public partial class ADOAudit_Template
   {
      ...
      {
         List<Audit_Template> ret = new List<Audit_Template>();
         string sql = "SELECT  Template_ID,Template_Code,Template_Key,Template_Name,Template_Remark,Stat,Template_Table,Template_Column,Template_ColumnType,Template_CheckSQL FROM Audit_Template where 1=1 AND ((Stat is null) or (Stat=0) ) order by Template_ID desc ";
          SqlDataReader dr=null;  
           try {  
          dr=(SqlDataReader)idb.ReturnReader(sql);
         while(dr.Read())
