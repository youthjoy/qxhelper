...
namespace QX.DAL
{
   ...
   public partial class ADOVerify_Nodes
   {
      ...
      {
         List<Verify_Nodes> ret = new List<Verify_Nodes>();
         string sql = "SELECT  VerifyNode_ID,VerifyNode_Code,VerifyNode_Name,VerifyNode_Remark,VerifyNode_AuditNum,Stat FROM Verify_Nodes where 1=1 AND ((Stat is null) or (Stat=0) ) order by VerifyNode_ID desc ";
          SqlDataReader dr=null;  
           try {  
          dr=(SqlDataReader)idb.ReturnReader(sql);
         while(dr.Read())
