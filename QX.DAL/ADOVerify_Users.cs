...
namespace QX.DAL
{
   ...
   public partial class ADOVerify_Users
   {
      ...
      {
         List<Verify_Users> ret = new List<Verify_Users>();
         string sql = "SELECT  VU_ID,VU_VerifyNode_Code,VU_User,VU_UserName,VU_Dept,VU_DeptName,VU_Duty,VU_VerifyTempldate_Code,Stat FROM Verify_Users where 1=1 AND ((Stat is null) or (Stat=0) ) order by VU_ID desc ";
          SqlDataReader dr=null;  
           try {  
          dr=(SqlDataReader)idb.ReturnReader(sql);
         while(dr.Read())
