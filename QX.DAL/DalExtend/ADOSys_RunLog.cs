...
namespace QX.DAL
{
   ...
   public partial class ADOSys_RunLog
   {
      ...
      {
         List<Sys_RunLog> ret = new List<Sys_RunLog>();
         string sql = "SELECT  Log_ID,Log_Date,Log_SQL,Log_Author,Log_Fun,Log_SQLParameter,Log_Error FROM Sys_RunLog where 1=1 AND ((Stat is null) or (Stat=0) ) order by Log_ID desc ";
          SqlDataReader dr=null;  
           try {  
          dr=(SqlDataReader)idb.ReturnReader(sql);
         while(dr.Read())
