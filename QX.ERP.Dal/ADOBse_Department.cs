...
namespace QX.ERP.DAL
{
   ...
   public partial class ADOBse_Department
   {
      ...
      {
         List<Bse_Department> ret = new List<Bse_Department>();
         string sql = "SELECT  Dept_ID,Dept_Code,Dept_Name,Dept_PCode,Dept_PName,Dept_Owner,Dept_OwnerName,Dept_OTel,Dept_OMob,Dept_Num,Dept_IsLeaf,Dept_Level,Dept_SimpleCode,Dept_SimpleName,Stat,Dept_Stat,Dept_Desp,Dept_Bak,Dept_Date FROM Bse_Department where 1=1 AND ((Stat is null) or (Stat=0) ) order by Dept_ID desc ";
          SqlDataReader dr=null;  
           try {  
          dr=(SqlDataReader)idb.ReturnReader(sql);
         while(dr.Read())
