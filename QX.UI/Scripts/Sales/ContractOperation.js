...
{ name: 'WHPP_Bak', index: 'WHPP_Bak', width: 100, align: 'center', sortable: false, hidden: false}],
    ...
    InitPay: function(dataurl, targetPreFix) {
        ...
        Contract_Operation.setupWH_PayGrid($('#' + gridobj), $('#' + pagerobj), $('#' + searchobj), dataurl);
    },
...
{ name: 'WHB_Date', index: 'WHB_Date', width: 100, align: 'center', sortable: false, hidden: false}],
    ...
    InitBanlance: function(dataurl, targetPreFix) {
        ...
        var pagerobj = targetPreFix + 'pager';
        var searchobj = targetPreFix + 'search';
        Contract_Operation.setupWH_BanlanceGrid($('#' + gridobj), $('#' + pagerobj), $('#' + searchobj), dataurl);
    },
...
{ name: 'PUM_BOwner', index: 'PUM_BOwner', width: 100, align: 'center', sortable: false, hidden: false}],
    ...
    ToolBarInit: function() {
        ...
        $toolbar.empty();
        $toolbar.append("<li id='PU_toolbar_rec' class='toolbar_add'><a onclick='Contract_Operation.CreateOrder()' href='#'>订单</a></li>");

    },
    ContractOperationFormID: 'ContractInfoForm',
    SaveData: function() {
        $("#ContractEditType").val("valid");
        if (!$('#' + Contract_Operation.ContractOperationFormID).validate().form()) {
            return false;
        }

        $("#" + Contract_Operation.ContractOperationFormID).submit();
    ...
    AfterSaveDraft: function(result) {
        if (result == "success") {
            if ($("#ContractEditType").val() == "draft") {
                ...
                window.location = "/Sales/QContractList/Cust";
            }
        } else {
            ShowMsg("操作失败!请重试");
        }
    ...
    //history.back();
        ...
        location = "/Sales/QContractList/Cust"+"?tab="+tab ;
    },
    InitPUGrid: function(dataurl, targetPreFix) {
        var gridobj = targetPreFix + 'grid';
        var pagerobj = targetPreFix + 'pager';
        var searchobj = targetPreFix + 'search';
        Contract_Operation.setupPU_MainGrid($('#' + gridobj), $('#' + pagerobj), $('#' + searchobj), dataurl);
    },
    OtherInit: function() {
        //变量初始化
    ...
    InitData: function() {
        $("#tabs").tabs();
        //Contract_Operation.OtherInit();
        //Contract_Operation.ToolBarInit();

        var moptions = {
            success: function(data) { if (data.result == "success") { ShowMsg("操作成功"); Contract_Operation.AfterSaveDraft(data.result); } },  // post-submit callback
            width: 800,
            timeout: 3000
        };

        //供应商基本信息
        $("#" + Contract_Operation.ContractOperationFormID).ajaxForm(moptions);
        ...
        if (formOper == "edit" && rCode != undefined) {
            ...
            //采购往来
            Contract_Operation.InitPUGrid("/Sales/PUList/" + rCode, "pu");
            //结算情况
            Contract_Operation.InitBanlance("/Sales/SupBanlanceList/" + rCode, "ban");
            //结算情况
            Contract_Operation.InitPay("/Sales/SupPayList/" + rCode, "pay");
