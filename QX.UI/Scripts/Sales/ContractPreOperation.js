...
{ name: 'PUM_BOwner', index: 'PUM_BOwner', width: 100, align: 'center', sortable: false, hidden: false}],
    ...
    AfterSaveDraft: function(result) {
        if (result == "success") {
            window.location = "/Sales/QPreContractList/PreCust";
        } else {
            alert("操作失败!请重试");
        }
    ...
    InitData: function() {
        ...
        var moptions = {
            success: function(data) { if (data.result == "success") { alert("成功"); Contract_Operation1.AfterSaveDraft(data.result); } },  // post-submit callback
            width: 800,
            timeout: 3000
        };
