...
var Supply_Operation =
{
    setupWH_PayGrid: function(grid, pager, search, dataurl) {
        grid.jqGrid({
            ...
            caption: '付款情况',
            footerrow: false,
            colNames: ['付款日期', '付款单位', '付款方式', '付款金额', '', '', '财务经手人', '采购经手人', '备注'],
...
{ name: 'WHPP_Bak', index: 'WHPP_Bak', width: 100, align: 'center', sortable: false, hidden: false}],
    ...
    InitPay: function(dataurl, targetPreFix) {
        ...
        Supply_Operation.setupWH_PayGrid($('#' + gridobj), $('#' + pagerobj), $('#' + searchobj), dataurl);
    },
...
{ name: 'WHB_Date', index: 'WHB_Date', width: 100, align: 'center', sortable: false, hidden: false}],
    ...
    InitBanlance: function(dataurl, targetPreFix) {
        ...
        var pagerobj = targetPreFix + 'pager';
        var searchobj = targetPreFix + 'search';
        Supply_Operation.setupWH_BanlanceGrid($('#' + gridobj), $('#' + pagerobj), $('#' + searchobj), dataurl);
    },
...
{ name: 'PUM_BOwner', index: 'PUM_BOwner', width: 100, align: 'center', sortable: false, hidden: false}],
    ...
    ToolBarInit: function() {
        ...
        $toolbar.empty();
        $toolbar.append("<li id='PU_toolbar_rec' class='toolbar_add'><a onclick='Supply_Operation.CreateOrder()' href='#'>订单</a></li>");

    },
    SupplyOperationFormID: 'WH_Supplier_Top_Form',
    SaveData: function() {

            
        if (!$('#' + Supply_Operation.SupplyOperationFormID).validate().form()) {
            return false;
        }

        $("#" + Supply_Operation.SupplyOperationFormID).submit();
    ...
    InitPUGrid: function(dataurl, targetPreFix) {
        ...
        var pagerobj = targetPreFix + 'pager';
        var searchobj = targetPreFix + 'search';
        Supply_Operation.setupPU_MainGrid($('#' + gridobj), $('#' + pagerobj), $('#' + searchobj), dataurl);
    },
    OtherInit: function() {
//        Common_Select_Dict.Init($('#WHS_Group'), 'SUPPLYGROUP');
    },
    InitData: function() {
        $("#tabs").tabs();
        Supply_Operation.OtherInit();
        Supply_Operation.ToolBarInit();

        var moptions = {
            success: function(data) { if (data.result == "success") { ShowMsg("成功"); history.back(); } },  // post-submit callback
            width: 800,
            timeout: 3000
        };

        //供应商基本信息
        $("#" + Supply_Operation.SupplyOperationFormID).ajaxForm(moptions);

        var formOper = $("#SupplyFormoper").val();
        ...
        if (formOper == "edit" && rCode != undefined) {
            ...
            //采购往来
            Supply_Operation.InitPUGrid("/Supply/PUList/" + rCode, "pu");
            //结算情况
            Supply_Operation.InitBanlance("/Supply/SupBanlanceList/" + rCode, "ban");
            //结算情况
            Supply_Operation.InitPay("/Supply/SupPayList/" + rCode, "pay");

        }
        else {
            $.ajax({
                type: 'post',
                url: '/Supply/GenerateSupplyCode',
